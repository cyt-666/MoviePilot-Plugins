...
def create_style_static_2(image_path, title, font_path, font_size=(170,75), font_offset=(0,40,40), blur_size=50, color_ratio=0.8, resolution_config=None, bg_color_config=None):
    try:
        ...
        zh_font = ImageFont.truetype(zh_font_path, int(max(1, round(float(zh_font_size)))))
        en_font = ImageFont.truetype(en_font_path, int(max(1, round(float(en_font_size)))))
        ...
        if not text_alpha_bbox:
            logger.warning("static_2 文字图层未产生可见像素，将使用最终图直绘兜底")
