...
class MediaCoverGenerator(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            def normalize_optional_config(value):
                if value is None:
                    return ""
            ...
            self._zh_font_url = normalize_optional_config(config.get("zh_font_url"))
            self._en_font_url = normalize_optional_config(config.get("en_font_url"))
            self._zh_font_path = normalize_optional_config(config.get("zh_font_path"))
            self._en_font_path = normalize_optional_config(config.get("en_font_path"))
            ...
            self._zh_font_custom = normalize_optional_config(config.get("zh_font_custom", ""))
            self._en_font_custom = normalize_optional_config(config.get("en_font_custom", ""))
    ...
    def __update_config(self):
        ...
        更新配置
        """
        def serialize_optional_path(value):
        ...
        self.update_config({
            ...
            "zh_font_url": serialize_optional_path(self._zh_font_url),
            "en_font_url": serialize_optional_path(self._en_font_url),
            "zh_font_path": serialize_optional_path(self._zh_font_path),
            "en_font_path": serialize_optional_path(self._en_font_path),
...
''',
    ...
    def __generate_image_from_path(self, server, library_name, title, image_path=None, config_bg_color=None):
        ...
        # 字体路径可能来自旧配置或被用户清理过；生成前再强制刷新一次，避免静默生成无文字封面。
        def is_valid_font_path(font_path, sample_text):
            if not font_path:
                return False
            try:
        ...
        if not is_valid_font_path(self._zh_font_path, "媒体库") or not is_valid_font_path(self._en_font_path, "MoviePilot"):
            logger.warning("字体路径未设置或无效，尝试重新获取字体")
            self.__get_fonts()

        if not is_valid_font_path(self._zh_font_path, "媒体库"):
            logger.error(f"主标题字体文件无效，无法生成带文字封面: {self._zh_font_path}")
            return False

        if not is_valid_font_path(self._en_font_path, "MoviePilot"):
            logger.error(f"副标题字体文件无效，无法生成带文字封面: {self._en_font_path}")
            return False
    ...
    def __get_fonts(self):
        ...
        zh_font_source = self._zh_font_custom or self._zh_font_url
        en_font_source = self._en_font_custom or self._en_font_url
        zh_custom_type = detect_string_type(zh_font_source)
        en_custom_type = detect_string_type(en_font_source)
        current_zh_font_url = zh_font_source if zh_custom_type == 'url' else default_zh_url
        ...
        for font_info in active_fonts_to_process:
            ...
            sample_text = "媒体库" if lang == "主标题" else "MoviePilot"
            ...
            if not using_local_font:
                ...
                if url_has_changed or not font_file_is_valid:
                    ...
                    # 使用安全的字体下载方法
                    if self.download_font_safely_with_timeout(url, downloaded_font_file_path, sample_text=sample_text):
                        try:
    ...
    def download_font_safely_with_timeout(self, font_url: str, font_path: Path, timeout: int = 60, sample_text: str = None) -> bool:
        """
        带超时的安全字体下载方法，避免首次下载时阻塞过久
        """
        try:
            logger.info(f"开始下载字体（超时限制: {timeout}秒）: {font_url}")
            return self.download_font_safely(font_url, font_path, retries=1, timeout=timeout, sample_text=sample_text)

        except Exception as e:
            logger.error(f"字体下载过程中出现异常: {e}")
            return False

    def download_font_safely(self, font_url: str, font_path: Path, retries: int = 2, timeout: int = 30, sample_text: str = None):
        """
