...
将 MoviePilot V3 的内置 Agent 工具通过 **MCP（Model Context Protocol）** 暴露给 ChatGPT、Codex、VS Code Copilot 和 Claude Desktop 等客户端，让 AI 助手可以直接查询订阅、管理下载、整理媒体库等。
...
- **按需 operation 说明**：提供只读 `moviepilot_api_describe`，Agent 可先查询单个 operation 的参数合同、字段值规则、用途边界和结果结构，再调用 `moviepilot_api`
...
- MoviePilot **V3 3.0.0+**（需要内置 `/api/v1/mcp` 端点）
...
转发；包装层额外提供只读 `moviepilot_api_describe`，用于查询单个 operation 的参数合同、字段值规则、用途边界和结果结构。
例如 `media.search` 明确区分候选搜索与 `media.recognize` 的身份解析，并说明前者的 `query.type` 只能使用
`media`、`music`、`collection` 或 `person`。`moviepilot_api` 的实际 operation 参数和权限仍由 MoviePilot V3 服务端校验。
...
- 如需限制能力，应在 MoviePilot V3 的工具暴露策略或上游服务端按 operation 进行控制
...
见 [package.v3.json](../../package.v3.json) 中的 `history` 字段。
