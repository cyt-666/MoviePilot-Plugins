...
class MoviePilotMCP(_PluginBase):
    ...
    @staticmethod
    def _moviepilot_operation_ids(schema: Any) -> List[str]:
        """从 V3 moviepilot_api Schema 提取稳定的 operation ID 列表。"""
        if not isinstance(schema, dict):
            return []
        properties = schema.get("properties")
        operation_schema = properties.get("operation_id") if isinstance(properties, dict) else None
        operation_ids = operation_schema.get("enum") if isinstance(operation_schema, dict) else None
        if not isinstance(operation_ids, list):
            ...
            for branch in schema.get("oneOf") or []:
                ...
                operation = branch_properties.get("operation_id") if isinstance(branch_properties, dict) else None
                operation_id = operation.get("const") if isinstance(operation, dict) else None
                if isinstance(operation_id, str):
                    operation_ids.append(operation_id)
        return sorted({operation_id for operation_id in operation_ids if isinstance(operation_id, str)})

    @classmethod
    def _compact_moviepilot_api_schema(cls, schema: Any) -> Any:
        """把 V3 网关的完整 oneOf 合同投影为外部客户端可转换的轻量合同。"""
        operation_ids = cls._moviepilot_operation_ids(schema)
        ...
        return {
            ...
            "properties": {
                ...
                "path_params": {
                    "type": "object",
                    "description": "Named route placeholder values for the selected operation.",
                    "additionalProperties": True,
                },
                "query": {
                    "type": "object",
                    "description": "Query-string values for the selected operation.",
                    "additionalProperties": True,
                },
                "body": {
                    "description": "Native JSON request value for the selected operation.",
                },
            },
    ...
    def _rewrite_tools_list_response(
        ...
        """改写 JSON-RPC tools/list 响应，并返回是否发生了改写。"""
        ...
        if not isinstance(result, dict) or not isinstance(result.get("tools"), list):
            return resp_data, False
        tools = result["tools"]
        rewritten = self._rewrite_tool_list(tools)
        if rewritten == tools:
            return resp_data, False
        ...
        return [
            {
                ...
                "content": [
                    ...
                    {"component": "VCardText", "text": "V3 MCP：完整工具能力由 MoviePilot 内置 MCP 统一提供"},
    ...
    async def handle_mcp(self, request: Request):
        ...
        # tools/list 仅在包装层兼容 V3 大 Schema，不裁剪工具能力
        if resp.status_code == 200:
            try:
                resp_data, changed = self._rewrite_tools_list_response(resp.json())
                if changed:
    ...
    async def handle_openapi_spec(self, request: Request):
        """动态生成包含全部工具的 OpenAPI 3.0.3 规范文档（无需认证）。"""
        if not self._enabled:
            raise HTTPException(status_code=403, detail="MoviePilot MCP 包装层未启用")
        ...
        spec = self._build_openapi_spec(tools, base_url)
    ...
    def _default_scopes(self) -> List[str]:
        return list(self._oauth_scopes)
