...
TRAKT_MEDIA_SOURCE = MediaSource("trakt")
...
class TraktSync(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        self.token = {}
        self._migrate_history_media_identity()
    ...
    def _on_discover_source(self, event: Event):
        ...
        event_data.extra_sources.append(
            DiscoverMediaSource(
                name="Trakt",
                media_source=TRAKT_MEDIA_SOURCE,
    ...
    def _page_items(page: Any) -> list:
        """读取 SDK 查询页中的已物化条目。"""
        items = page.get("items") if isinstance(page, dict) else getattr(page, "items", None)
        return list(items or [])

    def _load_download_history_by_hashes(self, hashes: List[str]) -> dict:
        """通过 V3 查询 SDK 按下载 Hash 读取稳定历史快照。"""
        history_map = {}
        for download_hash in hashes:
            page = list_download_history(
                filters=DownloadHistoryFilter(download_hash=str(download_hash)),
                page=QueryPageRequest(page=1, count=1),
            )
            items = self._page_items(page)
            if items:
                history_map[download_hash] = items[0]
        return history_map

    def _load_calendar_subscriptions(self) -> List[Any]:
        """通过 V3 查询 SDK 分页读取订阅快照，避免直接访问 ORM。"""
        subscriptions = []
        page_number = 1
        page_size = 200
        while True:
            page = list_subscriptions(
                page=QueryPageRequest(page=page_number, count=page_size)
            )
            items = self._page_items(page)
            subscriptions.extend(items)
            total = self._object_value(page, "total")
            if not items or len(items) < page_size or (
                total is not None and page_number * page_size >= int(total)
            ):
                break
            page_number += 1
    ...
    def _load_calendar_downloads(self) -> List[dict]:
        ...
        history_map = self._load_download_history_by_hashes(hashes) if hashes else {}
        records = []
        for torrent in torrents:
            ...
            media_source, media_id = resolve_media_identity(
                media_source=self._object_value(media, "media_source"),
                media_id=self._object_value(media, "media_id"),
            )
            if not media_source or not media_id:
                media_source, media_id = resolve_media_identity(
                    media_source=self._object_value(history, "media_source"),
                    media_id=self._object_value(history, "media_id"),
                )
    ...
    def _calendar_download_matches(self, item: dict, download: dict) -> bool:
        show_source, show_id = resolve_media_identity(
            media_source=MediaSource.TMDB,
            media_id=item.get("show_tmdb_id"),
        )
        task_source, task_id = resolve_media_identity(
            media_source=download.get("media_source"),
            media_id=download.get("media_id"),
        )
        text = str(download.get("text") or "")
        if task_source and task_id:
            if show_source != task_source or show_id != task_id:
                return False
        else:
    ...
    def _calendar_is_subscribed(self, item: dict, subscriptions: List[Any]) -> bool:
        ...
        season = self._as_int(item.get("season"))
        if not show_source or not show_id or season is None:
            return False
        for subscribe in subscriptions:
            media_source, media_id = resolve_media_identity(
                media_source=self._object_value(subscribe, "media_source"),
                media_id=self._object_value(subscribe, "media_id"),
            )
            if media_source != show_source or media_id != show_id:
                continue
    ...
    def _enrich_calendar_states(
        ...
        try:
            subscriptions = self._load_calendar_subscriptions()
        except Exception as exc:
    ...
    def _media_signature(
        ...
        item_type: Any,
        media_id: Any,
        media_source: Any = MediaSource.TMDB,
    ) -> Optional[str]:
        ...
        if not normalized_type or not source or not normalized_id:
            return None
        return f"{normalized_type}:{source.value}:{normalized_id}"

    @classmethod
    def _history_media_identity(
        cls,
        history: dict,
    ) -> Tuple[Optional[MediaSource], Optional[str]]:
        """读取同步历史的统一身份，并兼容旧版 TMDB 字段。"""
        media_source, media_id = resolve_media_identity(
        ...
        return resolve_media_identity(
            media_source=MediaSource.TMDB,
            media_id=history.get("tmdbid"),
        )

    @classmethod
    def _history_media_signature(cls, history: dict) -> Optional[str]:
        media_source, media_id = cls._history_media_identity(history)
        return cls._media_signature(
            history.get("type"),
            media_id,
            media_source,
        )

    def _migrate_history_media_identity(self) -> None:
        """幂等迁移插件旧同步历史，无法回填的记录原样保留。"""
        histories = self.get_data("history") or {}
        ...
        migrated_histories = {}
        changed = False
        for history_id, raw_history in histories.items():
            if not isinstance(raw_history, dict):
                migrated_histories[history_id] = raw_history
                continue
            history = dict(raw_history)
            media_source, media_id = self._history_media_identity(history)
            if media_source and media_id:
                normalized_source = media_source.value
                if (
                    history.get("media_source") != normalized_source
                    or str(history.get("media_id") or "").strip() != media_id
                    or "tmdbid" in history
                ):
                    history["media_source"] = normalized_source
                    history["media_id"] = media_id
                    history.pop("tmdbid", None)
                    changed = True
            migrated_histories[history_id] = history

        if changed:
            self.save_data("history", migrated_histories)
    ...
    def _history_signatures_for_source(self, source: str) -> set:
        ...
        for history in (self.get_data("history") or {}).values():
            if not isinstance(history, dict) or not self._history_source_matches(
                ...
                continue
            signature = self._history_media_signature(history)
            if signature:
    ...
    def _history_entry_matches(
        ...
        if not self._history_source_matches(history.get("source"), source):
            return False
        history_signature = self._history_media_signature(history)
    ...
    def _record_sync_history(
        ...
        media_source, media_id = resolve_media_identity(media=mediainfo)
        if not media_source or not media_id:
            logger.warning(f"Trakt 同步历史未写入：媒体身份无效，标识={stable_key}")
            return False
    ...
    def _subscription_exists(self, mediainfo, meta) -> bool:
        return bool(self.subscribechain.exists(mediainfo=mediainfo, meta=meta))
        ...
        try:
            ...
            if library_exists or self._subscription_exists(mediainfo, meta):
                ...
                if not sub_id:
                    ...
                    return False
                subscribe = get_subscription(sub_id)
                if subscribe:
